...
export default function CreateTokens(props) {
    ...
    // get gamesList
    useEffect(() => {
        axios.get(process.env.REACT_APP_DATABASE_API_GAME_URL)
            .then(function (data) {
    ...
    const createGame = (gameName) => {
        axios.post(process.env.REACT_APP_DATABASE_API_GAME_URL, {
            ...
            .then(function (data) {
                if (data.data.errors) {
                    ...
                    props.showAlert("Success", "Game has been successfully created", "", "success");
                }
            });
    }
    ...
    const createGameItem = (gameId, itemId, itemName, itemDescription, itemImage) => {
        axios.post(process.env.REACT_APP_DATABASE_API_NFT_URL, {
            apikey: process.env.REACT_APP_DATABASE_API_KEY,
            ...
            .then(function (data) {
                if (data.data.errors) {
                    ...
                    props.showAlert("Success", "Game item has been successfully created", "", "success");
                }
            });
    }
    ...
    const mintNFT = (receiverAddress, gameId, itemId) => {
        ...
        contract.methods.mint(receiverAddress, gameId, itemId).send({
            from: props.account
        }).then(function(result) {
            console.log(result);
            // TODO: add explorer tx link
            props.showAlert("Success", "NFT has been successfully minted", "", "success");
        });
    }
                    ...
                    <TabPanel value={value} index={1}>
                        ...
                        <form className={classes.root} noValidate autoComplete="off">
                            <TextField
                                ...
                                required
                            >
                                {gamesList && gamesList.map((option) => (
